...
#include "imu.hpp"
using namespace std;
//无锡北纬31°，海拔10m，估测g=9.793
const float g_wuxi = 9.793;
...
int IMU::OpenRS232USBModule()
{
	...
	{
		printf("/dev/ttyUSB10 opened correctly.\n");
	};
...
int IMU::OpenRS232USBModule_Interrupt(void signal_handler_IO(int))
{
	...
	saio.sa_handler = (signal_handler_IO); //指定信号处理函数
	saio.sa_flags = 0;
	// 信号类型，信号处理方式，旧方式
	sigaction(SIGIO, &saio, NULL);
	//allow the process to receive SIGIO
	// cout<<getpid()<<endl;
	// 参考：Linux-signal异步通知机制
	fcntl(this->fd, F_SETOWN, getpid());  //将进程pid传入内核
	fcntl(this->fd, F_SETFL, FASYNC);	  //设置异步通知
	cfsetispeed(&port_settings, B115200); // set baud rates
	port_settings.c_cflag = B115200 | CS8 | CREAD | CLOCAL;
	port_settings.c_iflag = IGNPAR;
	port_settings.c_oflag = 0;
...
float IMU::ToAngle(uint8_t *readbuff)
{
	// 判断第一字节的高四位，以区分正负
	if ((readbuff[0] >> 4) == 0)
		return ((readbuff[0] & 0x0F) * 100 + (readbuff[1] >> 4) * 10 + (readbuff[1] & 0x0F) + (float)((readbuff[2] >> 4) / 10.0) + (float)((readbuff[2] & 0x0F) / 100.0)) * M_PI / 180.0;
	else if ((readbuff[0] >> 4) == 1)
		return -((readbuff[0] & 0x0F) * 100 + (readbuff[1] >> 4) * 10 + (readbuff[1] & 0x0F) + (float)((readbuff[2] >> 4) / 10.0) + (float)((readbuff[2] & 0x0F) / 100.0)) * M_PI / 180.0;
}

float IMU::ToAngle(const std::vector<uint8_t> &readbuff, int index)
{
	if (readbuff[index] >> 4 == 0)
	{
		float temp = (readbuff[index] & 0x0F) * 100 + (readbuff[index + 1] >> 4) * 10 + (readbuff[index + 1] & 0x0F) + (float)((readbuff[index + 2] >> 4) / 10.0) + (float)((readbuff[index + 2] & 0x0F) / 100.0);
		return temp * M_PI / 180.0;
	}
	else if ((readbuff[index] >> 4) == 1)
	{
		float temp = -((readbuff[index] & 0x0F) * 100 + (readbuff[index + 1] >> 4) * 10 + (readbuff[index + 1] & 0x0F) + (float)((readbuff[index + 2] >> 4) / 10.0) + (float)((readbuff[index + 2] & 0x0F) / 100.0));
		return temp * M_PI / 180.0;
	}
...
{
	if ((readbuff[0] >> 4) == 0)
		return ((readbuff[1] >> 4) + (readbuff[1] & 0x0F) / 10.0 + (readbuff[2] >> 4) / 100.0 + (readbuff[2] & 0x0F) / 1000.0)*g_wuxi;
	else if ((readbuff[0] >> 4) == 1)
		return -((readbuff[1] >> 4) + (readbuff[1] & 0x0F) / 10.0 + (readbuff[2] >> 4) / 100.0 + (readbuff[2] & 0x0F) / 1000.0)*g_wuxi;
}

float IMU::ToAcc(const std::vector<uint8_t> &readbuff, int index)
{
	if ((readbuff[index] >> 4) == 0)
	{
		float temp = (readbuff[index + 1] >> 4) + (readbuff[index + 1] & 0x0F) / 10.0 + (readbuff[index + 2] >> 4) / 100.0 + (readbuff[index + 2] & 0x0F) / 1000.0;
		return temp * g_wuxi;
	}
	else if ((readbuff[0] >> 4) == 1)
	{
...
uint8_t IMU::CRC_cal(const std::vector<uint8_t> &buff, uint8_t len)
{
	uint8_t temp = 0;
	len++;
...
bool IMU::SetComFreq(string freq = "Freq_25Hz")
{
	...
	tcflush(this->fd, TCIOFLUSH); //清除所有正在发生的I/O数据
